...
`getsymprof` and the size of the resulting difference**; that the gateway's first
argument is always the dataset alias `` `profile `` (see the superseded note on Task 6);
