...
from core.connections import (DEFAULT_PROFILE_TABLE, Connection,
                              probe, profile_query, resolve_kind)
...
def test_the_gateway_is_probed_with_a_real_call_not_count():
    ...
    class Fake:
        ...
        def query(self, expr):
            sent.append((self.endpoint, expr))
            return 1
    ...
    profile_calls = [e for where, e in sent if where == "vprof:5100"]
    assert profile_calls == [profile_query(
        Connection(name="ap"), dt.date(2026, 7, 29), "000001.C2")]
    assert not any(e.startswith("count") for e in profile_calls)
